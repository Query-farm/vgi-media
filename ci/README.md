# CI: the vgi-media worker integration suite
...
vgi-media VGI worker through the **real DuckDB `vgi` extension** on every push /
PR.
...
1. **Build the worker** — `go build -o vgi-media-worker ./cmd/vgi-media-worker`.
   The resulting binary is a self-contained stdio worker the extension can
   spawn; `VGI_MEDIA_WORKER` (an absolute path) is the ATTACH `LOCATION`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) builds + starts the
   ...
   the preprocessed tree, points `VGI_MEDIA_WORKER` at the built worker binary,
